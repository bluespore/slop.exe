  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center gap-4 p-6">
      ...
      >
        ...
        <button
          type="submit"
          className="cursor-pointer border px-4 py-2 text-lg"
        >
          enter arena
        </button>
      </form>
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      <div className="text-center">
        <p className="text-3xl font-bold">LASER SWAMP</p>
        <p className="mt-2 text-gray-600">
          WASD to move · mouse to aim · click to fire
        </p>
        <p className="text-gray-600">last blob standing wins</p>
      </div>
      ...
      <p className="text-sm text-gray-500">
        the arena starts when every connected creature is ready
      </p>
    </div>
  );
}
...
function Countdown({ endsAt }: { endsAt: number | null }) {
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-2">
      <p className="text-sm tracking-widest">THE SWAMP STIRS</p>
...
function Arena({
  ...
  // A Vercel deploy can reach clients before the Cloudflare worker deploy.
  // Treat an older room snapshot as having no visible lasers rather than
  // crashing the whole page while those deployments catch up.
  const shots = Array.isArray(room.shots) ? room.shots : [];
  useEffect(() => {
    const sendMove = () =>
      move(
        (keys.current.has("d") ? 1 : 0) - (keys.current.has("a") ? 1 : 0),
        (keys.current.has("s") ? 1 : 0) - (keys.current.has("w") ? 1 : 0),
      );
  ...
  return (
    <div className="w-full">
      ...
      <div
        ...
        className="relative aspect-[3/2] w-full cursor-crosshair overflow-hidden border-2 border-black bg-lime-100 touch-none"
      >
        {Array.from({ length: room.width * room.height }, (_, index) => (
          <span
            key={index}
            style={{
              left: `${((index % room.width) / room.width) * 100}%`,
              top: `${(Math.floor(index / room.width) / room.height) * 100}%`,
            }}
            className="absolute h-px w-px bg-lime-300"
          />
        ))}
        {shots.map((shot) => (
          <Laser key={shot.id} shot={shot} room={room} />
        ))}
        {room.players
          .filter(
            (player) => player.alive && player.x !== null && player.y !== null,
          )
          .map((player) => {
            const x = player.x ?? 0;
            const y = player.y ?? 0;
            return (
              <div
                ...
                title={player.name}
                style={{
                  left: `${((x + 0.5) / room.width) * 100}%`,
                  top: `${((y + 0.5) / room.height) * 100}%`,
                }}
                className={`absolute flex size-7 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border-2 border-black text-xs font-bold ${player.id === playerId ? "bg-fuchsia-400" : "bg-cyan-300"}`}
              >
      ...
      <div className="mt-3 flex flex-wrap justify-center gap-x-4 gap-y-1 text-sm">
        {room.players.map((player) => (
          <span
            key={player.id}
            className={player.alive ? "" : "line-through text-gray-400"}
          >
...
function Laser({
  ...
  return (
    <span
      style={{
        left: `${((shot.x + 0.5) / room.width) * 100}%`,
        top: `${((shot.y + 0.5) / room.height) * 100}%`,
        width: `${(length / room.width) * 100}%`,
        transform: `rotate(${degrees}deg)`,
      }}
      className="absolute h-1 origin-left -translate-y-1/2 bg-red-500"
    />
  );
}
    ...
    <div className="flex flex-col items-center gap-6">
      <p className="text-4xl font-bold">
        {isYou
          ? "you own the swamp"
          : winnerName
            ? `${winnerName} owns the swamp`
            : "the swamp ate everyone"}
      </p>
      <button
        onClick={onPlayAgain}
        className="cursor-pointer border px-6 py-3 text-lg"
